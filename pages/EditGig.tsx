...
export const EditGig = () => {
    ...
    useEffect(() => {
        ...
        api.getGigById(gigId).then(gigData => {
            if (!gigData || gigData.venueId !== user.id) {
                // Not the owner or gig doesn't exist
                navigate('/venue/gigs');
                return;
            }
            setGig(gigData);
            setTitle(gigData.title);
            setDate(gigData.date);
            setTime(gigData.time);
            setBudget(String(gigData.budget));
            setGenres(gigData.genres);
            setDescription(gigData.description);
            setFlyerUrl(gigData.flyerUrl || '');
            setFlyerPreview(gigData.flyerUrl || null);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        try {
            await api.updateGig(gigId, user.id, {
                title,
                date,
                time,
    ...
    return (
        <div className="text-white min-h-full pb-20">
            <div className="sticky top-0 z-20 bg-black/80 backdrop-blur-sm p-4 flex items-center border-b border-zinc-800">
                ...
                <h1 className="font-orbitron text-xl font-bold text-white">Edit Gig</h1>
            </div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        {availableGenres.map(genre => (
                            <label key={genre} className={`p-2 text-center rounded-lg text-sm cursor-pointer border ${genres.includes(genre) ? 'bg-lime-400 text-black border-lime-400' : 'bg-zinc-800 border-zinc-700'}`}>
                                <input type="checkbox" value={genre} checked={genres.includes(genre)} onChange={handleGenreChange} className="sr-only" />
                                {genre}
                            </label>
                        ))}
                    </div>
