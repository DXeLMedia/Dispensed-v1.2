...
export const Settings = () => {
    ...
    const handleDownloadUsers = async () => {
        const users = await api.getDJs(); // A simple way to get a list of users
        const businesses = await api.getBusinesses();
        downloadCSV([...users, ...businesses], 'user_list.csv');
    };

    const handleDownloadDJs = async () => {
        const djs = await api.getDJs();
        const dataToExport = djs.map(({ tracks, mixes, ...dj }) => dj);
        downloadCSV(dataToExport, 'djs.csv');
    };

    const handleDownloadVenues = async () => {
        const businesses = await api.getBusinesses();
        downloadCSV(businesses, 'venues.csv');
    };

    const handleDownloadGigs = async () => {
        const gigs = await api.getGigs();
        downloadCSV(gigs, 'gigs.csv');
    };
    ...
    const handleSeedDatabase = async () => {
        if (window.confirm("Are you sure you want to seed the database? This will reset all in-memory data and download it as a single CSV file.")) {
            ...
            try {
                ...
                const combinedData = [
                    ...seededData.djs.map((item: any) => ({ dataType: 'dj', ...item })),
                    ...seededData.businesses.map((item: any) => ({ dataType: 'business', ...item })),
                    ...seededData.gigs.map((item: any) => ({ dataType: 'gig', ...item })),
                    ...seededData.tracks.map((item: any) => ({ dataType: 'track', ...item })),
                    ...seededData.playlists.map((item: any) => ({ dataType: 'playlist', ...item })),
                ];
